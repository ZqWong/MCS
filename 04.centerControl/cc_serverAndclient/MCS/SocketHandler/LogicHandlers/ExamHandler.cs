...
{
    class ExamHandler : HandlerInterface
    {
        ...
        public void MessageReceive(UserToken userToken, SocketModel message)
        {
            ...
            {
                case (byte)Protocol.Command.ExamStart:

                    ExcuteExamStartCommand(ip);

                    break;

                case (byte)Protocol.Command.ExamOver:

                    ExcuteExamOverCommand(ip);
        ...
        #region 消息分发执行
        private void ExcuteExamStartCommand(string ip)
        {
            主界面.GetSingleton().UpdateDgvState(ip, "ExamStart");
        }

        private void ExcuteExamOverCommand(string ip)
        {
            主界面.GetSingleton().UpdateDgvState(ip, "ExamOver");
        }
        #endregion
