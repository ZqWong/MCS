...
namespace MCS
{
    [DefaultEvent("PageIndexChanged"), DefaultProperty("RecordCount"), Description("C# WinForm下的分页用户控件"), ToolboxBitmap(typeof(PageControl), "Resources.Bitmap.pager")]
    public partial class PageControl : UserControl
    {
        ...
        private string PagerText = "总共{0}条记录，当前第{1}页，共{2}页，每页";
        private string PagerText2 = "条数据";
        ...
        //private void PageControl_MouseHover(object sender, EventArgs e)
        //{
        //    foreach (Control control in base.Controls)
        //    {
        //        if (control is Button)
        ...
        {
            // 总共{0}条记录，当前第{1}页，共{2}页，每页";
            this.SetPagerText();

            // _PageIndex 当前第几页（默认1，点击下一步等Button，随时更改）
            // _PageSize 每页显示多少记录（cbPageSize值）
            // _RecordCount 总记录数（后台查询后设置）
            // _PageCount 总页数（方法计算所得）
        ...
        {
            // 一共多少页
            _PageCount = GetPageCount(this._RecordCount, this._PageSize);

            //MessageBox.Show(_RecordCount.ToString());
            ...
            //MessageBox.Show(this._PageIndex.ToString() + "," + _PageCount.ToString());

            string[] strArray = new string[] { this._RecordCount.ToString(), this._PageIndex.ToString(), _PageCount.ToString() };
        ...
        {
            if (this._PageSize.ToString() != this.cbPageSize.Text)
            {
                //MessageBox.Show("PageIndex:" + this.PageIndex.ToString() + "," + "PageSize" + PageSize + "_PageCount:" + PageCount);

                this._PageSize = Convert.ToInt32(this.cbPageSize.Text);
