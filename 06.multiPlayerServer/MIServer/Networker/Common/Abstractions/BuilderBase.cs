...
namespace Networker.Common.Abstractions
{
		...
		where TBuilder : class, IBuilder<TBuilder, TResult> where TBuilderOptions : class, IBuilderOptions
	{
        ...
        //	return this as TBuilder;
        //}
        public TBuilder RegisterAllPacketHandler(Assembly assembly)
        {
            IEnumerable<Type> container = assembly.GetTypes()
            ...
            {
                var packet = handler.BaseType.GetGenericArguments();
                if (packet.Length == 1)
                {
                    var a = packet[0];
                    module.AddPacketHandler(packet[0], handler);
                }
            }
        ...
        public TBuilder UseSync(bool isSync)
        {
            options.IsSyncPattern = isSync;
            return this as TBuilder;
        }
