...
namespace SuperSocket.SocketBase.Logging
{
    ...
    public class Nloglog : ILog
    {
        ...
        private string debugTargerName = "debugTargetFile";
        private string infoTargerName = "infoTargetFile";
        private string errorTargerName = "errorTargetFile";
        private string fatalTargerName = "fatalTargetFile";
        ...
        public Nloglog(string name)
        {
            ...
            var debugFileTarget = new FileTarget(debugTargerName)
            {
                // FileName = "${basedir}/Logs/log${shortdate}.txt",
                FileName = "${basedir}Logs\\log${shortdate}_debug.txt",
            ...
            var vAsyncDebugTarget = new AsyncTargetWrapper(debugFileTarget);

            var infoFileTarget = new FileTarget(infoTargerName)
            {
                // FileName = "${basedir}/Logs/log${shortdate}.txt",
                FileName = "${basedir}Logs\\log${shortdate}_info.txt",
            ...
            var vAsyncInfoTarget = new AsyncTargetWrapper(infoFileTarget);

            var errorFileTarget = new FileTarget(errorTargerName)
            {
                // FileName = "${basedir}/Logs/log${shortdate}.txt",
                FileName = "${basedir}Logs\\log${shortdate}_error.txt",
            ...
            var vAsyncerrorTarget = new AsyncTargetWrapper(errorFileTarget);

            // Step 3. Define rules
            config.AddRule(LogLevel.Info, LogLevel.Fatal, vAsyncTargetConsole);
            config.AddRule(LogLevel.Trace, LogLevel.Debug, vAsyncDebugTarget);
            config.AddRule(LogLevel.Info, LogLevel.Info, vAsyncInfoTarget);
            config.AddRule(LogLevel.Warn, LogLevel.Fatal, vAsyncerrorTarget);
