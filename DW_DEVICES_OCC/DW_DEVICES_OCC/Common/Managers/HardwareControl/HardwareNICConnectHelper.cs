...
public class HardwareNICConnectHelper
{
    ...
    public class LinkConnection
    {
        ...
        public Action<string, string> ReceiveAction;
        public Action<string, string> ServerDownAction;
        ...
        public bool SendAsciiCode(string cmd)
        {
            if (InitConnection())
            {
                ...
                {
                    Debug.Warn($"LinkConnection sendAsciiCode cmd:{cmd} Failed, {e}");
                    return false;
                }
                finally
                {
        ...
        public bool SendBytes(byte[] cmd)
        {
            if (InitConnection())
            {
                ...
                {
                    Debug.Warn(string.Format("设备控制网口数据异常: " + e.Message));
                    return false;
                }
                finally
                {
                ...
                _stream.BeginRead(data, 0, data.Length,
                    asyncResult =>
                    {
                        try
                        {
                            ...
                            ReceiveAction?.Invoke(returndata.Trim(), _name);
                            Recive();
                        }
                        catch (SocketException e)
                        {
                            if (e.ErrorCode == 10054)
                            {
                                ...
                                ServerDownAction?.Invoke("服务器已断线", _name);
                            }
                            else
                            {
                                //OnErr(e.Message, _name);
                                ErrorAction?.Invoke(e.Message, _name);
                            }
                        }
                        catch (Exception ex)
                        {
                            //OnErr(ex.Message, _name);
                            ErrorAction?.Invoke(ex.Message, _name);
                        }
                    }, null);
            }
            catch (Exception ex)
