...
public static class PathHelper
{
    ...
    /// </summary>
    /// <param name="directoryPath"></param>
    public static void DeleteFolder(string directoryPath, List<string> exceptPathList)
    {
        if (!Directory.Exists(directoryPath))
        {
            return;
        ...
        foreach (string d in Directory.GetFileSystemEntries(directoryPath))
        {
            if (File.Exists(d))
            {
                ...
                {
                    Debug.Info(string.Format("删除文件 {0} 失败. error {1}", d, e.Message.ToString()));
                }
    ...
    /// <param name="fullName"></param>
    /// <returns></returns>
    public static string GetParentFolderName(string fullName)
    {
        string parentPath;
